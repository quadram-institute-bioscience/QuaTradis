...
class TestPrepareEMBLFile(unittest.TestCase):
	...
	def test_small_valid_file(self):
		...
		p = PrepareEMBLFile(plotfiles, 1, 4, 2, 15, None,False,dynamic_window_params)
	...
	def test_reuse_annotation(self):
		...
		dynamic_window_params.pop("dynamic_window")
		plotfiles= [os.path.join(data_dir, 'valid'),os.path.join(data_dir, 'valid_ctrl')]
		p = PrepareEMBLFile(plotfiles, 1, 4, 2, 15, os.path.join(data_expand_genes_dir, 'one_gene'),False,dynamic_window_params)
		self.assertTrue(p.create_file())
	...
	def test_prepare_embl_dynamic_window(self):
		...
		dynamic_window_params.pop("dynamic_window")
		plotfiles= [os.path.join(data_dir, 'valid'),os.path.join(data_dir, 'valid_ctrl')]
		p = PrepareEMBLFile(plotfiles, 1, 4, 2, 15, os.path.join(data_expand_genes_dir, 'one_gene'),True,dynamic_window_params)
		self.assertTrue(p.create_file())
