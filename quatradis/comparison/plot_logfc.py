...
class PlotLogOptions:
    def __init__(self, genome_length, minimum_logfc, maximum_pvalue, maximum_qvalue, minimum_logcpm, window_size, span_gaps, report_decreased_insertions, minimum_block):
...
class PlotLog:
    def __init__(self, comparison_filename, embl_file, options, output_plot_filename="output.plot", output_scores_filename="output.pqvals"):
        self.comparison_filename = comparison_filename
        self.embl_file = embl_file
        self.options = options
        self.plot_filename = output_plot_filename
        self.scores_filename = output_scores_filename
    ...
    def genome_wide_logfc(self, logfc_coord_values):
        logfc_to_bases = numpy.zeros(self.options.genome_length, dtype=float)
        pval_to_bases = numpy.zeros(self.options.genome_length, dtype=float)
        qval_to_bases = numpy.zeros(self.options.genome_length, dtype=float)
    ...
    def span_block_gaps(self, logfc_to_bases, pval_to_bases, qval_to_bases):
        ...
        if self.options.span_gaps <= 0:
            return logfc_to_bases, pval_to_bases, qval_to_bases

        # span blocks if they are close together
        for b in blocks:
            span_index = b.end + (self.options.window_size * self.options.span_gaps)
            if span_index >= self.options.genome_length:
                continue

            span_value = numpy.absolute(logfc_to_bases[span_index])
            if span_value >= self.options.minimum_logfc:
